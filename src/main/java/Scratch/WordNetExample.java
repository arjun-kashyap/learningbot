...
public class WordNetExample {
    ...
    public static void main(String[] args) {
        ...
        String propsFile = "/Users/Suresh/Arjun STEM/demo/src/main/resources/jwnl-properties.xml";//args[0];
        try {
    ...
    public void go() throws JWNLException {
        ...
        demonstrateSymmetricRelationshipOperation2(FUNNY, DROLL);
    ...
    private void demonstrateSymmetricRelationshipOperation2(IndexWord start1, IndexWord end1) throws JWNLException {
        // find all synonyms that <var>start</var> and <var>end</var> have in common
        IndexWord start = Dictionary.getInstance().lookupIndexWord(POS.ADJECTIVE, "curable");
        IndexWord end = Dictionary.getInstance().lookupIndexWord(POS.ADJECTIVE, "nonlethal");
        RelationshipList list = RelationshipFinder.getInstance().findRelationships(start.getSense(1), end.getSense(1), PointerType.SIMILAR_TO);
        System.out.println("Synonym relationship2 between \"" + start.getLemma() + "\" and \"" + end.getLemma() + "\":");
        for (Iterator itr = list.iterator(); itr.hasNext();) {
            ((Relationship) itr.next()).getNodeList().print();
        }
        if (list.size() != 0) {
